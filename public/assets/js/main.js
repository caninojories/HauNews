	 ...
	 $.ajax({
        ...
        method: "GET"
				...
				$.ajax({
					...
					method: 'GET',
					data: {skip:0, limit:100, status:$status}
				}).success(function(response){
					...
					var $postList = response;
				 	var news = '<div class="row">';
						...
						if(content.length > 150){
							...
							// 	more += '<hr>';
							// 	more += '<span class="morenews">...view all post...</span>';
							// 	more += '<hr>';
							// 	more += '</div>';
...
//============= headine news ================
  $.ajax({
    ...
    if(response){
    	$('.home-headline').removeClass('hidden');
      ...
      var topTenArray = '<ul class"list-group">';
			if(window.location.pathname === '/') {
				for(var topNews in response){
					if(response[topNews].department === 'null'){
						$('.top-10-news').removeClass('hidden');
						topTenArray += '<li class="list-group-item">'+ i++ +'<a href="/post/'+response[topNews].postId+'"> '+response[topNews].title+'</a></li>';
